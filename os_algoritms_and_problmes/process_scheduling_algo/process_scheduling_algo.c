...
// method 7 for Longest Job First algoritm
...
// method 8 for Longest Remaining Time First algoritm
...
void lrtf(){
    ...
    while(j<n)
    {
        ...
        if(index!=previous && response_time[index]==-1)
        {
            response_time[index]=counter-arrival_time[index];
        }
        
        counter++;
        
        if(Bt[index]==0 && index!=-1)
        {
            completion_time[index]=counter;
            j++;
        }
        previous=index;
    }
    ...
    printf("\nProcess Table:\nPid\tAT\tBT\tCT\tTAT\tWT\tRT\n");
    for(i=0;i<n;i++)
    printf("P%d\t%d\t%d\t%d\t%d\t%d\t%d\n",(i+1),arrival_time[i],burst_time[i],completion_time[i],turnaround_time[i],waiting_time[i],response_time[i]);
...
// method 9 for HIGHEST RESPONSE RATIO NEXT
void hrrn(){
    ...
    for(i=0;i<n;i++)
    scanf("%d",&arrival_time[i]);
    printf("Burst time of the Processes:\n");
    for(i=0;i<n;i++)
    scanf("%d",&burst_time[i]);
    int completion_time[n],to_check[n];
    for(i=0;i<n;i++)
    to_check[i]=0;
    ...
    while(j<n)
    {
        ...
        for(i=0;i<n;i++)
        {
            RR=((float)(counter-arrival_time[i]+burst_time[i]))/burst_time[i];
            ...
            if(RR>max)
            {
                if(arrival_time[i]<=counter && to_check[i]==0)
                {
                    max=RR;
                    index=i;
                }
    ...
    float sum=0;
    for(i=0;i<n;i++)
    sum+=(float)turnaround_time[i];
    printf("\nAverage Turnaround Time: %f",sum/n);
    sum=0;
    for(i=0;i<n;i++)
    sum+=(float)waiting_time[i];
...
int main()
{
	...
	printf("Press 7 for 'LONGEST JOB FIRST' \n");
	printf("Press 8 for 'LONGEST REMAINING TIME FIRST' \n");
	printf("Press 9 for 'HIGHEST RESPONSE RATIO NEXT' \n");
